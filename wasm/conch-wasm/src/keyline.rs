...
mod tests {
    ...
    fn backspace_deletes_previous_char() {
        ...
        // a, ab, a (backspace), ac
        assert_eq!(states.last().unwrap().text, "ac");
        assert_eq!(states.last().unwrap().cursor, 2);
    }
    ...
    fn escaped_backslash() {
        ...
        // a, a\, a\b
        assert_eq!(states.last().unwrap().text, "a\\b");
    }
    ...
    #[test]
    fn long_csi_with_params_is_skipped() {
    ...
    #[test]
    fn multiple_backspaces_clear_all() {
    ...
    #[test]
    fn insert_mid_line_with_arrows() {
    ...
    #[test]
    fn ctrl_c_mid_typing_preserves_buffer() {
        let states = process("abc\\x03def");
        // Find the ctrl-c state
        let ctrl_c = states.iter().find(|s| s.event == "ctrl-c").unwrap();
